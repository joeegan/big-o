 ...
 */
export default function binarySearch(target, array, min=0, max=array.length-1, iterationCount=1) {
  let indexesOfInterest = _.range(min, max + 1);
  const guessIndex = Math.floor((max + min) / 2);
  const guessValue = array[guessIndex];
  emit = _.partialRight(emit, { array, target, guessIndex, guessValue, indexesOfInterest });
