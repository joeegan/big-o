...
function oLogN(n) {
  ...
  for (let i = 2; i < n + 3; i++) {
    ...
    results.push([
      i - 2,
      binarySearch(0, arr).iterationCount,
    ])
  }
  return results
}
...
function oNLogN(n) {
  ...
  for (let i = 1; i < n + 1; i++) {
    results.push([
      i,
      quickSort(new Array(i)).iterationCount,
    ])
  }
  return results
}
...
window.onload = function() {
  ...
  google.charts.setOnLoadCallback(() => {
    ...
    chart(
      document.getElementById('chart_div_on_two'),
      oNTwo(200)
    )
    chart(
      document.getElementById('chart_div_o_log_n'),
      oLogN(200)
    )
    chart(
      document.getElementById('chart_div_o_n_log_n'),
      oNLogN(20)
    )
  })
}
